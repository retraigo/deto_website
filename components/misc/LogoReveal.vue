...
<template>
    <div>
        <div ref="topcontainer" class="top-container group">
            <div class="main-content" @mousemove="movedaro" @touchmove="touchdaro" @mouseenter="() => (isTouched = true)"
                @mouseleave="onLeave" @touchstart="() => (isTouched = true)" @touchend="onLeave">
                <div :class="`movingPointer transition-opacity duration-500`" ref="movingthing" :style="{
            ...
            <div ref="toptext" style=""
                ...
                <span class="font-azonix tracking-wider text-2xl lg:text-3xl mt-8">Easwari Engineering College
                    <br />(Autonomous)</span>
                <span class="font-azonix tracking-wider text-lg mt-8">Department of Artificial Intelligence & Data Science
                    <br />Presents</span>
            </div>
...
function move(e: { pageX: number; pageY: number }) {
    ...
    let logoGradient = `radial-gradient(circle at ${e.pageX - textRect.left
    ...
    logoGradient = `radial-gradient(circle at ${e.pageX - textRect.left
        }px ${e.pageY - textRect.top}px, black 40%, transparent 100%)`;
